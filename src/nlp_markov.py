"""Uses markov chains to generate text."""

import os
import errno
import sqlite3
import random
import utils.m_parser as pa
...
class MarkovNet(object):
    ...
    def __init__(self):
        ...
        '''
        There are a lot of databases to be created, one pair for each gram.
        If we want to store words up to a depth of 3, we need 4 databases to
        store word pairs or triplets and one for the dictionary itself. The
        dictionary pairs word ids to words and the other databases relate id
        pairs/groups to their frequency.

        :param brain:
        '''
        self.dictionary = sqlite3.connect(
            os.path.join(MarkovNet.DATA_DIR, self.brain + '_dict.db'))
        self.fu_list = sqlite3.connect(
            os.path.join(MarkovNet.DATA_DIR, self.brain + '_fu.db'))
        self.ru_list = sqlite3.connect(
            os.path.join(MarkovNet.DATA_DIR, self.brain + '_ru.db'))

        self.contractions = os.path.join(
            MarkovNet.DATA_DIR, self.brain + '_cntr.csv')
    ...
    def print_dict(self):
        ...
        if self.dt_curs.fetchone() is None:
    ...
    def print_dict_tostring(self, filt="None"):
        ...
        output = ""
        if filt == "None":
            for row in self.dt_curs.execute(
                    'SELECT * FROM dictionary ORDER BY wID'):
                output += str(row)
                output += "\n"
        elif filt == "Class":
            for row in self.dt_curs.execute(
                    'SELECT * FROM dictionary WHERE class IS NULL'):
                output += str(row)
                output += "\n"
        return output

    def fu_search(self, wordA, next):
        query = self.fu_curs.execute(
            'SELECT * FROM uForwardNet WHERE wordA=? AND next=?',
            (wordA, next,))
    ...
    def train(self, message):
        ...
        print('Sentence learned, to undo type !undo.')

    def classify(self, word="xnull", classification="xna"):
